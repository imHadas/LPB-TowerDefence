    ...
    public class GameModel : ModelBase
    {
        ...
        private async Task FireTower()
        {
            ...
            foreach (Tower tower in rp.Towers)
            {
                ...
                for (int i = Math.Max((int)tower.Coords.x - (int)tower.Range, 0); i <= Math.Min((int)tower.Coords.x + (int)tower.Range, Table.Size.x - 1); i++)
                {
                    for (int j = Math.Max((int)tower.Coords.y - (int)tower.Range, 0); j <= Math.Min((int)tower.Coords.y + (int)tower.Range, Table.Size.y - 1); j++)
                    {
                        foreach (Unit unit in Table[i, j].Units)
                        {
                            if (unit.Owner != rp)
